...
% thresh = 0.01;
% 
% fuzzy_means_mean_J_values = zeros(1, largest_m - smallest_m + 1);
% q = 2;
% 
...
%         seed = seeds(repeat);
%         [U, theta, J] = fuzzy_c_means(X, m, q, thresh, seed);
%         J_values_for_m(repeat) = J;
%     end
%     fuzzy_means_mean_J_values(m - smallest_m + 1) = mean(J_values_for_m);
% end
% 
% k_means_m_values = smallest_m:largest_m;
% figure;
% plot(k_means_m_values, fuzzy_means_mean_J_values, '-o', 'LineWidth', 2, ...
%     'MarkerSize', 6);
% xlabel('Number of Clusters (m)');
% ylabel('Cost Function (J)');
% title('fuzzy c-means Clustering: m vs J (averaged over 10 runs)');
% grid on;
...
tic
    ...
    figure;
    imagesc(im_cl_label);
    title(sprintf("fuzzy c-means with %d clusters\n" + ...
        "(not preprocessed)", m))
    ...
    fprintf("\n\nfuzzy c-means with %d clusters not preprocessed\n", m);
    fprintf('Adjusted Rand Index (ARI): %.4f\n', AR);
    fprintf('Rand Index (RI): %.4f\n', RI);
    fprintf('Mirkin Index (MI): %.4f\n', MI);
    fprintf('Hubert Index (HI): %.4f\n', HI);
